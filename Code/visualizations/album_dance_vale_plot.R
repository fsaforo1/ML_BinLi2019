...
tots_thom <- filter(tots_thom, !album_name %in% non_studio_albums) %>% 
  select(track_name, album_name, loudness, album_release_year) %>% 
  arrange(-loudness)
...
val <- ggplot(tots_thom, aes(x = loudness, y = as.character(album_release_year))) + 
  geom_density_ridges() +
  ...
  ggtitle('Does loudness really matter?') +
  labs(x = 'Song loudness', y = '') +
  scale_x_continuous(breaks = c(-24, -18, -12, -6,  0)) +
  scale_y_discrete(labels = album_names_label)

ggsave(val, filename = str_glue('valenceplot.png'), width = 4, height = 3)
background <- image_read(str_glue('valenceplot.png'))
background
